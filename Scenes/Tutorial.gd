	...
	if event is InputEventMouseMotion && Input.is_action_pressed("right_click"):
		...
		if state == 0:
			...
			for child in $PoemLabelContainer.get_children():
				child.text = "Available actions are\nin the upper left\nof the screen"
			$GUI/Right/PlayerOptions/Walk.disabled = false
...
func display_character_options(_player):
	if _player == true: 
		...
		if whose_turn.food_contacts.size() == 0:
			$GUI/Right/PlayerOptions/PickUp.disabled = true
		if state == 0 || state == 3:
			$GUI/Right/PlayerOptions/Walk.disabled = true
		$GUI/Right/PlayerOptions/Wait.disabled = true
		if state == 4: 
			for child in $PoemLabelContainer.get_children():
				child.text = "Ice cream cones\ncontain texts;\ngo read one"
...
func spawn_ice_cream(pos):
	var ice_cream_node = load("res://Scenes/ClusterIceCream.tscn")
	var new_ice_cream = ice_cream_node.instance()
	add_child(new_ice_cream)
